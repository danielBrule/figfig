...
class SitemapURLs(Base):
    ...
    newspaper_id = Column(Integer, ForeignKey("dbo.newspaper.id"), nullable=False)
...
class ArticlesURLs(Base):
    ...
    source_id = Column(Integer, ForeignKey("dbo.sitemap_urls.id"), nullable=False)
    stage = Column(Integer, ForeignKey("dbo.article_stage.id"), nullable=False)
    insert_date = Column(DateTime, nullable=False)


class Keywords(Base):
    __tablename__ = "keywords"
    __table_args__ = (Index("keywords_full_keyword", "full_keyword"), {"schema": "dbo"})
    id = Column(Integer, primary_key=True, autoincrement=True)
...
class Articles(Base):
    ...
    __table_args__ = {"schema": "dbo"}
    id = Column(Integer, ForeignKey("dbo.articles_urls.id"), primary_key=True)
...
class ArticleKeywords(Base):
    ...
    __table_args__ = {"schema": "dbo"}
    article_id = Column(
        Integer, ForeignKey("dbo.articles.id"), nullable=False, primary_key=True
    )
    keyword_id = Column(
        Integer, ForeignKey("dbo.keywords.id"), nullable=False, primary_key=True
    )
...
class Comments(Base):
    ...
    parent_id = Column(
        String(100), ForeignKey("dbo.comments.id"), nullable=True, default=None
    )
    article_id = Column(Integer, ForeignKey("dbo.articles.id"), nullable=False)
    insert_date = Column(DateTime, nullable=False)
